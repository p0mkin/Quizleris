...
function saveQuizToStorage(quizData: Quiz): void {
...
function loadQuizFromStorage(quizId: string): Quiz | null {
...
function renderAnswers(q: Question) {
...
function renderStatus(message: string, kind: "neutral" | "correct" | "incorrect") {
    const colorClass =
...
function initializeQuiz(quizData: Quiz): void {
...
function renderQuestionCounter(): void {
    if (!quiz) return;
...
function renderNextButton(): void {
...
function initAdminElements(): void {
...
function promptAdminPassword(): boolean {
...
async function processOCRImage(_imageFile: File): Promise<{ prompt: string; choices: string[] } | null> {
...
function parseOCRText(text: string): { prompt: string; choices: string[] } {
    const lines = text
        .split(/\r?\n/)
        .map((l) => l.trim())
        .filter((l) => l.length > 0);
...
function setupAdminEvents(): void {
