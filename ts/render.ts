...
export function renderAnswers(q: Question): void {
    ...
    (window as any).renderMathInElement(answersContainer, {
        delimiters: [
            { left: "\\(", right: "\\)", display: false },
            { left: "\\[", right: "\\]", display: true },
        ],
 ...
 */
function renderMCQ(q: Question): void {
    ...
    if (allowMultiple && !isAnswered) {
        ...
        submitBtn.onclick = () => {
            const selected = Array.from(answersContainer.querySelectorAll(".answer-btn.selected"))
                .map(b => (b as HTMLElement).dataset.choiceId!);
            onAnswer(selected);
        };
...
function toggleMCSelection(qId: string, choiceId: string, btn: HTMLElement, multiple: boolean): void {
    if (!quiz) return;
    ...
    if (multiple) {
        if (current.includes(choiceId)) {
            current = current.filter((id: string) => id !== choiceId);
...
function renderNumeric(q: Question): void {
    const isAnswered = quiz?.hasAnswered && quiz.quiz.mode !== 'exam';
    ...
    input.oninput = () => {
        if (quiz) quiz.userAnswers.set(q.id, input.value);
        if (quiz?.quiz.mode === 'exam') renderExamNavigation();
    };

    if (quiz?.quiz.mode !== 'exam' && !isAnswered) {
        input.onkeydown = (e) => { if (e.key === 'Enter') onAnswer(input.value); };
    }
 ...
 */
function renderFillBlank(q: Question): void {
    const isAnswered = quiz?.hasAnswered && quiz.quiz.mode !== 'exam';
...
function renderTrueFalse(q: Question): void {
    const isAnswered = quiz?.hasAnswered && quiz.quiz.mode !== 'exam';
    const currentAnswer = quiz?.userAnswers.get(q.id);
    ...
    options.forEach(opt => {
        ...
        if (!isAnswered) {
            ...
            if (q.isTrue === opt.val) btn.classList.add("correct");
            if (currentAnswer === opt.val && q.isTrue !== opt.val) btn.classList.add("incorrect");
            btn.disabled = true;
        }
        answersContainer.appendChild(btn);
    });
...
function renderTextInput(q: Question): void {
    const isAnswered = quiz?.hasAnswered && quiz.quiz.mode !== 'exam';
    const currentVal = quiz?.userAnswers.get(q.id) || "";
...
function renderImageUpload(q: Question): void {
    const isAnswered = quiz?.hasAnswered && quiz.quiz.mode !== 'exam';
 ...
 */
export function onAnswer(answer: any): void {
    if (!quiz || (quiz.hasAnswered && quiz.quiz.mode !== 'exam')) return;
...
export function renderQuiz(): void {
    ...
    if (headerTitle) { headerTitle.textContent = quiz.quiz.title; }
...
export function renderExamNavigation(): void {
    if (!quiz) return;
        ...
        if (q.isLastQuestion) {
            ...
            btn.onclick = () => {
                ...
                const confirmMsg = unanswered > 0 ?
                    ...
                    t('quiz.examConfirm');
                if (confirm(confirmMsg)) showResults();
            };
        } else {
            btn.textContent = t('quiz.nextQuestion');
            btn.onclick = () => {
